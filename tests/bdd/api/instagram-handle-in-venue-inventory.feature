...
Feature: Serve the Instagram handle with each venue inventory row
  ...
  Scenario: Return an accepted handle with its confidence and tier
    Given the venue "Champagne Club" has an accepted Instagram handle
      "champagne_recifee" found by the "venue_website" tier with confidence 0.78
  ...
  Scenario: Report a low-confidence handle as such
    Given the venue "Casa Duvidosa" has a low-confidence Instagram handle
      "casaduvidosa" with confidence 0.55
    When I list the venue inventory
  ...
  Scenario: Leave every other inventory field untouched
    Given the venue "Champagne Club" has an accepted Instagram handle
      "champagne_recifee" found by the "venue_website" tier with confidence 0.78
    When I list the venue inventory
    Then the row for "Champagne Club" still reports its venue id, name, address,
  ...
  Scenario: Survive a malformed stored record
    ...
    And the venue "Champagne Club" has an accepted Instagram handle
      "champagne_recifee" found by the "venue_website" tier with confidence 0.78
    When I list the venue inventory
  ...
  Scenario: Tolerate a record stored before the source field existed
    Given the venue "Bar Antigo" has an Instagram handle "barantigo" with no
      recorded source
    When I list the venue inventory
